<template>
  <div class="about">
        <header class="header">
        <div class="container header-container">
            <div class="header-logo">
                <a class="logo-link" href="index.html">
                    <img class="logo-img" src="img/logo.png" alt="logo">
                    BRAN<span class="pink">D</span>
                </a>
            </div>

            <div class="browse">
                <div class="browse-btn">
                    <button>Brows <i class="fas fa-caret-down"></i></button>
                    <div class="drop_nav">
                        <h3 class="d_n_heading">Women</h3>
                        <ul class="d_n_menu">
                            <li><a href="#">Dresses</a></li>
                            <li><a href="#">Tops</a></li>
                            <li><a href="#">Sweaters/Knits</a></li>
                            <li><a href="#">Jackets/Coats</a></li>
                            <li><a href="#">Blazers</a></li>
                            <li><a href="#">Denim</a></li>
                            <li><a href="#">Leggings/Pants</a></li>
                            <li><a href="#">Skirts/Shorts</a></li>
                            <li><a href="#">Accessories</a></li>
                        </ul>
                        <h3 class="d_n_heading">Men</h3>
                        <ul class="d_n_menu">
                            <li><a href="#">Tees/Tank tops</a></li>
                            <li><a href="#">Shirts/Polos</a></li>
                            <li><a href="#">Sweaters</a></li>
                            <li><a href="#">Sweatshirts/Hoodies</a></li>
                            <li><a href="#">Blazers</a></li>
                            <li><a href="#">Jackets/vests</a></li>
                        </ul>
                    </div>
                </div>
                <form action="#" class="search">
                    <input class="search-field" type="text" placeholder="Search for Item...">
                    <button class="search-btn"><img src="img/search.png" alt="search"></button>
                </form>
            </div>

            <div class="login">
                <a href="#"><img src="img/cart.png" alt=""></a>
                <div class="account"><button>My Account <i class="fas fa-caret-down"></i></button>
                    <div class="drop_cart">
                        <div class="drop_cart_one_product">
                          <img src="img/hhd/rectangle_24_1821.jpg" alt="#">
                            <div>
                                <h4>Rebox Zane</h4>
                                <ul class="stars">
                                    <li><i class="fas fa-star"></i></li>
                                    <li><i class="fas fa-star"></i></li>
                                    <li><i class="fas fa-star"></i></li>
                                    <li><i class="fas fa-star"></i></li>
                                    <li><i class="far fa-star"></i></li>
                                </ul>
                                <span>1 x $250</span>
                            </div>
                            <div class="actoon">
                              <button class="d_c_a">
                                <i class="fas fa-times-circle"></i></button></div>
                        </div>
                        <div class="drop_cart_one_product">
                          <img src="img/hhd/rectangle_24_1830.png" alt="#">
                            <div>
                                <h4>Rebox Zane</h4>
                                <ul class="stars">
                                    <li><i class="fas fa-star"></i></li>
                                    <li><i class="fas fa-star"></i></li>
                                    <li><i class="fas fa-star"></i></li>
                                    <li><i class="far fa-star"></i></li>
                                    <li><i class="far fa-star"></i></li>
                                </ul>
                                <span>1 x $250</span>
                            </div>
                            <div class="actoon">
                              <button class="d_c_a">
                                <i class="fas fa-times-circle"></i></button></div>
                        </div>
                        <div class="drop_cart_total_price">
                            <h5>TOTAL</h5>
                            <h5>$500.00</h5>
                        </div>
                        <div class="drop_cart_action">
                            <a class="drop-cart-ac" href="checkout.html">Checkout</a>
                            <a class="drop-cart-ac" href="shopping-cart.html">Go to cart</a>

                        </div>
                    </div>
                </div>
            </div>

        </div>

    </header>
    <div class="container">
        <nav>
            <ul>
                <li><a class="menu" href="index.html">home</a></li>
                <li><a class="menu" href="product.html">man</a></li>
                <li><a class="menu" href="single-page.html">woman</a></li>
                <li><a class="menu" href="shopping-cart.html">kids</a></li>
                <li><a class="menu" href="checkout.html">accoseriese</a></li>
                <li><a class="menu" href="#">featured</a></li>
                <li><a class="menu" href="#">hot deals</a></li>
            </ul>
            <div class="mega_nav">
                <div>
                    <h3 class="d_n_heading">Women</h3>
                    <ul class="d_n_menu mega_nav_ul">
                        <li><a href="#">Dresses</a></li>
                        <li><a href="#">Tops</a></li>
                        <li><a href="#">Sweaters/Knits</a></li>
                        <li><a href="#">Jackets/Coats</a></li>
                        <li><a href="#">Blazers</a></li>
                        <li><a href="#">Denim</a></li>
                        <li><a href="#">Leggings/Pants</a></li>
                        <li><a href="#">Skirts/Shorts</a></li>
                        <li><a href="#">Accessories</a></li>
                    </ul>
                </div>
                <div>
                    <h3 class="d_n_heading">Women</h3>
                    <ul class="d_n_menu mega_nav_ul">
                        <li><a href="#">Dresses</a></li>
                        <li><a href="#">Jackets/Coats</a></li>
                        <li><a href="#">Sweaters/Knits</a></li>
                        <li><a href="#">Tops</a></li>
                    </ul>
                    <h3 class="d_n_heading">Women</h3>
                    <ul class="d_n_menu mega_nav_ul">
                        <li><a href="#">Dresses</a></li>
                        <li><a href="#">Tops</a></li>
                        <li><a href="#">Sweaters/Knits</a></li>
                    </ul>
                </div>
                <div>
                    <h3 class="d_n_heading">Women</h3>
                    <ul class="d_n_menu mega_nav_ul">
                        <li><a href="#">Dresses</a></li>
                        <li><a href="#">Jackets/Coats</a></li>
                        <li><a href="#">Sweaters/Knits</a></li>
                        <li><a href="#">Tops</a></li>
                    </ul>
                    <img src="img/hhd/layers%20(1).jpg" alt="sale">
                </div>
            </div>
        </nav>
    </div>
    <main>
        <div class="pink_background_all_pages">
            <div class="container product_h1">
                <h1>New Arrivals</h1>
                <div class="breadcrumbs">
                    <ol>
                        <li class="breadcrumbs_li"><a href="#">Home</a></li>
                        <li class="breadcrumbs_li"><a href="#">Men</a></li>
                        <li class="breadcrumbs_li"><a href="#">New Arrivals</a></li>
                    </ol>
                </div>
            </div>
        </div>
        <article class="container basket">
            <div class="header_bascet">
                <span>Product Details</span>
                <span>unite Price</span>
                <span>Quantity</span>
                <span>shipping</span>
                <span>Subtotal</span>
                <span>ACTION</span>
            </div>
            <cart-list></cart-list>
            <div class="s_c_button">
              <button>cLEAR SHOPPING CART</button><button>cONTINUE sHOPPING</button></div>
        </article>
        <aside class="container s_c_form">
            <div>
                <form class="adress_form" action="#">
                    <h4>Shipping Adress</h4>
                    <div class="select"><select class="f-points f_p_s">
                            <option>Bangladesh</option>
                            <option>Bangladesh</option>
                            <option>Bangladesh</option>
                            <option>Bangladesh</option>
                            <option>Bangladesh</option>
                            <option>Bangladesh</option>
                            <option>Bangladesh</option>
                            <option>Bangladesh</option>
                        </select></div>
                    <input class="f-points" type="text" placeholder="State">
                    <input class="f-points" type="number" placeholder="Postcode / Zip">
                    <input class="sub_form" type="submit" value="get a quote">
                </form>
            </div>
            <div>
                <form class="cupon_form" action="#">
                    <h4>Shipping Adress</h4>
                    <h5>Enter your coupon code if you have one</h5>
                    <input class="f-points" type="text" placeholder="State">
                    <input class="sub_form" type="submit" value="Apply coupon">
                </form>
            </div>
            <div class="total">
                <h6>Sub total<span>$900</span></h6>
                <h4>GRAND TOTAL<span class="total_pink">$900</span></h4>
                <hr>
                <button>proceed to checkout</button>
            </div>
        </aside>

    </main>
  </div>
</template>
<script>

import CartList from '../components/CartList.vue';

export default {
  components: { CartList },
};
</script>
